...
+ (instancetype)objectWithJSONData:(NSData *)data
  ...
  return [self
      objectWithKeyValues:[NSJSONSerialization JSONObjectWithData:data
                                                          options:kNilOptions
                                                            error:nil]
                    error:error];
}
  ...
  if ([self isSubclassOfClass:[NSManagedObject class]] && context) {
    return [[NSEntityDescription
        insertNewObjectForEntityForName:NSStringFromClass(self)
                 inManagedObjectContext:context] setKeyValues:keyValues
                                                      context:context
                                                        error:error];
  }
...
+ (instancetype)objectWithFilename:(NSString *)filename
  ...
  return [self
      objectWithFile:[[NSBundle mainBundle] pathForResource:filename ofType:nil]
               error:error];
}
...
+ (instancetype)objectWithFile:(NSString *)file
  ...
  return
      [self objectWithKeyValues:[NSDictionary dictionaryWithContentsOfFile:file]
                          error:error];
}
...
- (instancetype)setKeyValues:(id)keyValues
  ...
  @try {
    ...
    [[self class] enumeratePropertiesWithBlock:^(MJProperty *property,
                                                 BOOL *stop) {
      // 0.检测是否被忽略
      ...
      if (!type.isFromFoundation && typeClass) {
        value =
            [typeClass objectWithKeyValues:value context:context error:error];
      } else if (objectClass) {
        // 3.字典数组-->模型数组
        ...
        if (typeClass == [NSURL class]) {
          ...
          if ([type.code isEqualToString:MJTypeBOOL]) {
            ...
            if ([lower isEqualToString:@"yes"] ||
                [lower isEqualToString:@"true"]) {
              value = @YES;
            } else if ([lower isEqualToString:@"no"] ||
    ...
    if ([self respondsToSelector:@selector(
                                     keyValuesDidFinishConvertingToObject)]) {
...
+ (NSArray *)objectArrayWithJSONData:(NSData *)data
  ...
  return [self objectArrayWithKeyValuesArray:[NSJSONSerialization
...
+ (NSArray *)objectArrayWithKeyValuesArray:(id)keyValuesArray
  ...
  // 1.判断真实性
  MJAssertError([keyValuesArray isKindOfClass:[NSArray class]], nil, error,
                @"keyValuesArray参数不是一个数组");
...
+ (NSArray *)objectArrayWithFilename:(NSString *)filename
  ...
  return
      [self objectArrayWithFile:[[NSBundle mainBundle] pathForResource:filename
                                                                ofType:nil]
                          error:error];
}
...
+ (NSArray *)objectArrayWithFile:(NSString *)file
  ...
  return
      [self objectArrayWithKeyValuesArray:[NSArray arrayWithContentsOfFile:file]
                                    error:error];
}
...
- (NSDictionary *)keyValuesWithError:(NSError *__autoreleasing *)error {
  ...
  @try {
    ...
    // 转换完毕
    if ([self respondsToSelector:@selector(
                                     objectDidFinishConvertingToKeyValues)]) {
...
+ (NSArray *)keyValuesArrayWithObjectArray:(NSArray *)objectArray
                                     error:(NSError *__autoreleasing *)error {
  // 0.判断真实性
  MJAssertError([objectArray isKindOfClass:[NSArray class]], nil, error,
                @"objectArray参数不是一个数组");
